...
airdrop_helicopter_scnd = "I_Heli_Transport_02_F";
//airdrop_chance = 100;
...
diag_log "╔═════════════════════════════╗";
diag_log "║ AIRDROP wurde manuell gestartet !!";
diag_log "╚═════════════════════════════╝";
...
// AIRDROP STARTEN
//[[1,""],"life_fnc_ligaNews",true,false] spawn life_fnc_MP;
//sleep 300;
...
_mygroup1 addWaypoint [[380.47,267.8,0], 0];
...
sleep 0.1;
_var=floor(random 3);
if (_var==1) then {_box addWeaponCargoGlobal ["arifle_MX_SW_Black_F", 2];};
sleep 0.1;
...
sleep 0.1;
if (_var==2) then {_box addWeaponCargoGlobal ["arifle_Katiba_GL_F", 2];};
sleep 0.1;
if (_var==0) then {_box addWeaponCargoGlobal ["LMG_Mk200_F", 2];};
sleep 0.1;
if (_var==2) then {_box addWeaponCargoGlobal ["LMG_Zafir_F", 2];};
sleep 0.1;
...
sleep 0.1;
if (_var==1) then {_box addItemCargoGlobal ["HandGrenade_Stone", 5];};
sleep 0.1;
