...
private["_items","_abm","_e","_privatTank","_space","_itemWeight","_classNameLife","_vehicle", "_controll", "_liter", "_completeliter", "_costs", "_vehicleFuelold", "_vehicleArray", "_ui", "_progress", "_pgText", "_cP", "_vehicleName"];
_privatTank = SEL(_this,0);
_vehicleArray = nearestObjects[getPos player, ["Landvehicle"],7];
...
RPT("_classNameLife",_classNameLife);
...
if(_privatTank) then {
	...
	/**/
	_Fahrtank 	= round ((_completeliter - (_vehicleFuelold * _completeliter)) / 30); // 20 = Faktor
...
if(_action) then {
	...
	while {fuel _vehicle < 0.99} do {
		...
		if(_cP >= 1) exitWith {};
		if(player distance _vehicle > 7) exitWith {hint "Du musst zum Tanken im Umkreis von 7 Metern bleiben."; 5 cutText ["","PLAIN"];};
	...
	if (fuel _vehicle > 0.96) then {
		...
		if(_privatTank) then {
			...
			["ben",(round(((1 - _vehicleFuelold) * _completeliter) / 30)),true,true,false]call life_fnc_ligaWHbuy;
		};	
	};
	5 cutText ["","PLAIN"];	
