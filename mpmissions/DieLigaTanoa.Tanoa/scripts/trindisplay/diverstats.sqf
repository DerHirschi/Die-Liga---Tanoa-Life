...
_sacRT 		= 25; 
//_tankSize 	= 0;
_maxppO 	= 1.5; //Max ppO2 (atm). Determines the Maximum Operating Depth of the Diver.
_percO2 	= 0.21; //Percentage of Oxygen in breathing mix.
_percN2 	= 0.79; //Percentage of Nitrogen in breathing mix.
_percHe 	= 0.00; //Percentage of Helium in breathing mix.
_tempC 		= 24; //Temperature at surface (Celcius)

//_airTotal = _percO2 +_percN2 +_percHe;
//_psi = _tankSize; //Starting PSI of a full tank.
_psi 		= 0; //Starting PSI of a full tank.
//_psi = profileNamespace getVariable ["ligaprslft",0];
...
private "_pGridPos";
//12 cutRsc ["trin_disp","PLAIN"];
while {alive _trinDiver /*&& _psi >= 0 && player isEqualTo _trinDiver */ } do
{
	
	
	//scopeName "uw_loop";
	if (underwater _trinDiver && vest player in ["V_RebreatherIR","V_RebreatherB","V_RebreatherIA"] && /*_psi >= 0 &&*/ player isEqualTo _trinDiver) then
		{				
			if!(_tr)then {12 cutRsc ["trin_disp","PLAIN"]; _tr = true;playSound "trin_dispStart";};
			
			if ( _psi <= 0 || press_tausch) then {
				press_tausch = false;
				if(!isNil "li_prl") then {
					if(li_prl > 0) then {
						li_prl = li_prl - 1;
						_psi = 3000;
						if(li_prl isEqualTo 0) then { hint "Du hast keine Pressluftflasche mehr... Die letzte wird jetzt verwendent."; };
			...
			(_displayUI displayCtrl 1118) ctrlSetTextColor [0, 0, 0, 1];	
			(_displayUI displayCtrl 1115) ctrlSetTextColor [0, 0, 0, 1];	
			
			uiSleep 0.5; //---DO NOT CHANGE --- All time based calculations dependent on value.		
				
			if (_timeleft < 300 && _timeleft > 60) then {				
				playSound "trin_dispStart";
				(_displayUI displayCtrl 1115) ctrlSetTextColor [0.95, 0.89, 0, 1];	
			};
			if (_timeleft < 60) then {
				playSound "trin_dispWarn";	
				(_displayUI displayCtrl 1115) ctrlSetTextColor [0.95, 0.05, 0, 1];	
			};
				
			if (_psi < 200 && _psi > 50) then {				
				playSound "trin_dispStart";
				(_displayUI displayCtrl 1118) ctrlSetTextColor [0.95, 0.89, 0, 1];					
			};	
			if (_psi < 50) then {				
				playSound "trin_dispWarn";
				(_displayUI displayCtrl 1118) ctrlSetTextColor [0.95, 0.05, 0, 1];					
			};	
			...
			{				
				_decoTime = _AscCeil *(round(_totalTis *1));	// 4			
				_doDeco = 1;
				_diveCnt = _diveCnt - 1;
				playSound "trin_dispStart";
				uiSleep 0.1;
			...
			{				
				...
				_deepStopTime = _AscCeil *(round(_totalTis *1));	 // 3.5			
				playSound "trin_dispStart";
				uiSleep 0.1;
				playSound "trin_dispStart";
				_doDeepStop = 1;
			...
			if (_psi <= 0) then {				
				for [{_i=1},{_i<=4 && underwater _trinDiver && _psi <= 0},{_i=_i+1}] do {
					if(!isNil "li_prl") then {
						if(li_prl > 0) then {
							li_prl = li_prl - 1;
							_psi = 3000;
							...
							hint "ACHTUNG !!! Deine Pressluftflasche ist leer ... Du wirst ertrinken!!!";									
							for [{_w=1},{_w<=30 && underwater _trinDiver},{_w=_w+1}] do {
								(_displayUI displayCtrl 1118) ctrlSetTextColor [0.95, 0.05, 0, 1];	
								(_displayUI displayCtrl 1115) ctrlSetTextColor [0.95, 0.05, 0, 1];	
								playSound "trin_dispWarn";
								uiSleep 0.25;
					...
					if(_i isEqualTo 4 && underwater _trinDiver && _psi <= 0 ) then {
		...
		if(_tr) then {
			...
			_tr = false;
			playSound "trin_dispExit";
			//profileNamespace setVariable ["ligaprslft",_psi];
		};		
	};					
};
