...
if(_curTarget isKindOf "House_F" && (playerSide == west || cop_m || liga_cop_on)) exitWith {
	cop_m = false;
...
if!(_houseCfg isEqualTo []) exitWith {
	...
	if(!(_curTarget in life_vehicles) OR isNil {_curTarget GVAR "house_owner"}) then {
		...
		if((typeOf _curTarget) in [/*"Land_i_Garage_V1_F","Land_i_Garage_V2_F",*/"Land_GarageShelter_01_F"]) then {
			...
			_Btn3 ctrlShow true;
			if((typeOf _curTarget) in ["Land_House_Big_02_F","Land_House_Big_03_F"]) then {
...
_houseCfg = [(typeOf _curTarget)] call LIGACL_fnc_LigaLadenCfg;

if!(_houseCfg isEqualTo []) exitWith {
	
	if(!(_curTarget in life_vehicles) OR isNil {_curTarget GVAR "house_owner"}) then {
		if(_curTarget in life_vehicles) then {life_vehicles = life_vehicles - [_curTarget];};
		if!([_curTarget,(SEL(_houseCfg,3))]call LIGACL_fnc_buyCheck)exitWith {closeDialog 0;};
...
if((typeOf _curTarget) in ["Land_FuelStation_01_pump_F","Land_FuelStation_02_pump_F"]) exitWith {
	if(!(_curTarget in life_vehicles) OR isNil {_curTarget GVAR "house_owner"}) then {
		if(_curTarget in life_vehicles) then {life_vehicles = life_vehicles - [_curTarget];};
		/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
		...
		if((typeOf _curTarget) in ["Land_FuelStation_01_pump_F","Land_FuelStation_02_pump_F"]) then {
