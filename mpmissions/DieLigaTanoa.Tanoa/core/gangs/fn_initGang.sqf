...
if(playerSide in [civilian,independent] && (liga_cop_on || liga_lrk_on)) exitWith {}; //What in the hell?

[player] join (createGroup civilian);
{    	if(local _x && {(count units _x == 0)}) then {      deleteGroup _x;       	};           } foreach allGroups;
