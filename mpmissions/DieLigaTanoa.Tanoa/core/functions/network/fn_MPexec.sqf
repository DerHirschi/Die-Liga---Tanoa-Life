...
#define SKY_FW(SETTING) getArray(missionConfigFile >> "MPExec" >> SETTING)
...
//if(!(["life_fnc_",_functionName] call BIS_fnc_inString) && !(["liga_fnc_",_functionName] call BIS_fnc_inString) && !(["liga_fn_",_functionName] call BIS_fnc_inString) &&/* {!(["SPY_fnc_",_functionName] call BIS_fnc_inString)} && *//* {!(["DB_fnc_",_functionName] call BIS_fnc_inString)} && */ {!(["TON_fnc_",_functionName] call BIS_fnc_inString)} /*&&{!(toLower(_functionName) in ["bis_fnc_execvm","bis_fnc_effectkilledairdestruction","bis_fnc_effectkilledairdestructionstage2"])} && *//*{!(["SOCK_fnc_",_functionName] call BIS_fnc_inString)}*/) exitWith {false};
if(isServer && !hasInterface && {!(toLower(_functionName) in (SKY_FW("Server") + SKY_FW("Client")))}) exitWith {
				//	[_callerName,(format["unerlaubter remEXEC Versuch via PVEVH SERVER.. Von: %4  UID: %5 FNC: %1   TARG: %3  PARAM: %2",_functionName,_params,_target,_callerName,_callerUID])] remoteExecCall ["SPY_fnc_notifyAdmins",RANY];
					[_callerName,_callerUID,"unerlaubter remEXEC Versuch auf Server .. fnc_MPexec",(format[" FNC %1   \n TARG %3  \n PARAM %2",_functionName,_params,_target])] remoteExecCall ["SPY_fnc_cookieJar",RSERV];
					false;
				};

if(!isServer && {!(toLower(_functionName) in (SKY_FW("Client")))}) exitWith {
				//	[_callerName,(format["unerlaubter remEXEC Versuch via PVEVH CLIENT.. Von: %4  UID: %5 FNC: %1   TARG: %3  PARAM: %2",_functionName,_params,_target,_callerName,_callerUID])] remoteExecCall ["SPY_fnc_notifyAdmins",RANY];
					[_callerName,_callerUID,"unerlaubter remEXEC Versuch auf Client .. fnc_MPexec",(format[" FNC %1   \n TARG %3  \n PARAM %2",_functionName,_params,_target])] remoteExecCall ["SPY_fnc_cookieJar",RSERV];

					false;
					};					
if(toLower(_functionName) isEqualTo "db_fnc_asynccall") exitWith {	
	//[_callerName,(format["unerlaubter zugiffsversuch auf die DB.. UID: %3 Name: %2  PARAM: %1",_params,_callerName,_callerUID])] remoteExecCall ["SPY_fnc_notifyAdmins",RANY];
	[_callerName,_callerUID,"unerlaubter Zugiffsversuch auf die DB !!",(format["PARAM %1",_params])] remoteExecCall ["SPY_fnc_cookieJar",RSERV];
...
if(_functionName isEqualTo "bis_fnc_execvm") then {
	...
	if(_param2 != "initPlayerServer.sqf") exitWith {	
	//[_callerName,(format["unerlaubte Parameter für bis_fnc_execvm.. UID: %3 Name: %2  PARAM: %1",_params,_callerName,_callerUID])] remoteExecCall ["SPY_fnc_notifyAdmins",RANY];
	[_callerName,_callerUID,"unerlaubte Parameter für bis_fnc_execvm",(format["PARAM %1",_params])] remoteExecCall ["SPY_fnc_cookieJar",RSERV];
...
if(_callerUID != "__SERVER__" && _callerName != "__SERVER__" && toLower(_functionName) in ["spy_fnc_cookiejar","spy_fnc_notifyadmins"]) then {
	...
	if(toLower(_functionName) isEqualTo "spy_fnc_cookiejar") exitWith {
		private["_reportUID"];
		_reportUID = _params select 1;
		if(_reportUID != _callerUID) exitWith {
			if(isServer && _mode isEqualTo 0) then {
				[_callerName,_callerUID,"False reporting to SKY-Net (cheater)",(format["PARAM %1 /n REPORTER UID %2",_params,_reportUID])] remoteExecCall ["SPY_fnc_cookieJar",RSERV];
	...
	if(_callerName != _reportName) exitWith {
		if(isServer && _mode isEqualTo 0) then {
			[_callerName,_callerUID,"False reporting to SKY-Net (cheater)",(format["PARAM %1 /n REPORTER NAME %2",_params,_reportName])] remoteExecCall ["SPY_fnc_cookieJar",RSERV];
		};
		_exitScope = true;
	};
};
...
if(toLower(_functionName) isEqualTo "bis_fnc_endmission") exitWith {
	//[_callerName,(format["unerlaubter remEXEC Versuch bis_fnc_endmission.. UID: %3 Name: %2  PARAM: %1",_params,_callerName,_callerUID])] remoteExecCall ["SPY_fnc_notifyAdmins",RANY];
	[_callerName,_callerUID,"unerlaubter remEXEC Versuch bis_fnc_endmission..",(format["PARAM %1",_params])] remoteExecCall ["SPY_fnc_cookieJar",RSERV];
	
	false;
	}; //Don't allow BIS_fnc_endMission to be passed.
