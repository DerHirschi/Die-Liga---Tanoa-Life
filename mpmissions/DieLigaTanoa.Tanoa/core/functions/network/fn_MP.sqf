...
with missionnamespace do {
	...
	if(isServer && isDedicated) then {
		...
		if(toLower(_functionName) in (SKY_FW("Server") + SKY_FW("Client"))) then {
			life_fnc_MP_packet = [0,_params,_functionName,_target,_isPersistent,_isCall,profileName,getPlayerUID player];
		}else{
			[profileName,steamid,"unerlaubter remExec verusch.. fnc_MP",(format ["FNC %1    PARM %2",_functionName,_params]),player] remoteExec ["SPY_fnc_cookieJar",RSERV];
			SKYNET_END
		};
	};
