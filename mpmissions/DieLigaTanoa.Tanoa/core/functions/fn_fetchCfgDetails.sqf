...
_muzzles = [];
_magazines = [];
if(EQUAL(_entity,"")) exitWith {[]};
_cfg = if(isNil {_this select 1}) then
{
	switch (true) do
	{
		case (isClass (configFile >> CONFIG_MAGAZINES >> _entity)) : {CONFIG_MAGAZINES;};
		case (isClass (configFile >> CONFIG_WEAPONS >> _entity)) : {CONFIG_WEAPONS;};
		case (isClass (configFile >> CONFIG_VEHICLES >> _entity)) : {CONFIG_VEHICLES;};
		case (isClass (configFile >> CONFIG_GLASSES >> _entity)) : {CONFIG_GLASSES;};
	};
}
	else
{
...
_ret = [];
if(!(EQUAL(typeName _cfg,typeName "STRING")) OR {!isClass(configFile >> _cfg >> _entity)} OR {EQUAL(_cfg,"")}) exitWith {[]};
/*
