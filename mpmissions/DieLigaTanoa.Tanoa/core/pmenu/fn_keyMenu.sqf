...
for "_i" from 0 to (count life_vehicles)-1 do {
	...
	if(!isNull _veh && alive _veh && !((typeOf _veh) in ["Land_ShelvesMetal_F"])) then {
		_color = SEL(SEL(M_CONFIG(getArray,CONFIG_LIGA_VEHICLES,(typeOf _veh),"textures"),(_veh GVAR "Life_VEH_color")),0);
		...
		_name = getText(configFile >> CONFIG_VEHICLES >> (typeOf _veh) >> "displayName");
		_pic = getText(configFile >> CONFIG_VEHICLES >> (typeOf _veh) >> "picture");
...

{
	...
	{
		_plist lbAdd format["%1 - %2",_x GVAR["realname",name _x], side _x];
