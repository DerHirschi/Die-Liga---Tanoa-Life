...
if(!(_unit isKindOf "AllVehicles")) exitWith {hint "Du kannst hier kein GPS-Tracker anbringen"};
if(_unit isKindOf "Man") exitWith { hint "Da ist kein Gewinde dran !!!"};
if(player distance _unit > 7) exitWith {hint "Du musst in der Nähe bleiben !"};
...
if(player distance _unit > 7) exitWith {titleText["Du bist zu weit entfernt !!","PLAIN"];};
titleText["Du hast ein GPS-Tracker an das Fahrzeug befestigt !!","PLAIN"];
