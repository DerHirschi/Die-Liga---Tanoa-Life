...
*/
private["_itemList","_index","_config","_priceTag","_itemArray","_color"];
...
if((GVAR_UINS ["Weapon_Magazine",0]) == 1) then {
	//_config = M_CONFIG(getArray,"WeaponShops",_shop,"mags");
	{
		if(SEL(_x,0) in (uiNamespace getVariable ["Magazine_Array",[]])) then {
			...
			_color = ([(_x select 0),false]call life_fnc_ligaWeapWH select 1);
			_itemList lbSetColor  [(lbSize _itemList)-1, _color];
