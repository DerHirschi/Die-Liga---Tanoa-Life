...
if(!(EQUAL(_shopSide,""))) then {
	...
	if(playerSide in [civilian,independent]) then {		
		if(liga_cop_on) then {_flag = "cop";};	
		if(liga_lrk_on) then {_flag = "med";};	
	}else{	
...
(findDisplay 3100) displaySetEventHandler ["KeyDown","if((_this select 1) == 1) then {closeDialog 0; [] call life_fnc_ligaSetClothText;}"]; //Fix Custom Skin after ESC

sliderSetRange [3107, 0, 360];

//Cop / Civ Pre Check
if((SEL(_this,3) in ["bruce","dive","reb","kart"] && (playerSide in [west,east] || liga_cop_on || liga_lrk_on))) exitWith {hint localize "STR_Shop_NotaCiv"; closeDialog 0;};
if((SEL(_this,3) == "reb" && !lci_reb)) exitWith {hint localize "STR_Shop_NotaReb"; closeDialog 0;};
if((SEL(_this,3) in ["cop"] && (Side player != west || !liga_cop_on))) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
if((SEL(_this,3) in ["dive"] && !lci_dive)) exitWith { hint localize "STR_Shop_NotaDive"; closeDialog 0;};
if((SEL(_this,3) in ["alac"] && playerSide != east)) exitWith {hint "Du bist kein ALAC Mitarbeiter"; closeDialog 0;};

if(EQUAL(LIGA_SETTINGS(getNumber,"clothing_box"),1)) then {
	_pos = [1214.61,630.334,0.00176883];
} else {
...
if(EQUAL(LIGA_SETTINGS(getNumber,"clothing_box"),0)) then {
...
if!(EQUAL(SEL(_var,0),"")) then
{
	if(!(GVAR_MNS (SEL(_var,0)))) exitWith {hint format[localize "STR_Shop_YouNeed",[SEL(_var,0)] call life_fnc_varToStr]; closeDialog 0;};
};
...
[]spawn {uiSleep 5;[] call life_fnc_ligaSetClothText;};