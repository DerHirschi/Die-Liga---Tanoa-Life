...
if(playerSide isEqualTo civilian) then {
	_fish = (nearestObjects[getPos vehicle player,["Fish_Base_F"],13]);
}else{
	_fish = (nearestObjects[getPos vehicle player,["Fish_Base_F"],18]);
};
...
if(count _fish == 0) exitWith {titleText[localize "STR_NOTF_NetDropFail","PLAIN"]; life_net_dropped = false;};
{
	...
	if((playerSide isEqualTo civilian) && _forEachIndex > round ((count _fish) / 2 )) exitWith {}; 
} foreach (_fish);
