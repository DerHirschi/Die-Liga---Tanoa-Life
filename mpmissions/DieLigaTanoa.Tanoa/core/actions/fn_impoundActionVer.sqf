...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if(EQUAL(playerSide,east))then{
		[0,"STR_NOTF_BeingImpoundedALACver",true,[SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
	}else{
		[0,"STR_NOTF_BeingImpoundedver",true,[SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
	};
	...
	_type = FETCH_CONFIG2(getText,CONFIG_VEHICLES,(typeOf _vehicle),"displayName");
		...
		[_vehicle,true,player] remoteExecCall ["TON_fnc_vehicleStoreVer",RSERV];
