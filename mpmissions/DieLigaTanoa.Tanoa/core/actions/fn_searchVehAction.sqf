...
if((_vehicle isKindOf "Car") || !(_vehicle isKindOf "Air") || !(_vehicle isKindOf "Ship")) then
{
	...
	//_inventory = [(_vehicle GVAR "vehicle_info_inv")] call fnc_veh_inv;
	//if(isNil {_inventory}) then {_inventory = "Nothing in storage."};
	_owners = [_owners] call life_fnc_vehicleOwners;
	_tuv = _vehicle GVAR ["tuv",[0,0]];
	_tuv = SEL(_tuv,0);
	if(_vehicle isKindOf "Car") then {_tuv = 14 - _tuv;} else {_tuv = 7 - _tuv;};	
	
	if(_owners isEqualTo "any<br/>") then
	{
		_owners = "No owners, impound it<br/>";
	};
	hint parseText format ["<t color='#FFBF00'><t size='2.2'><t align='center'>Fahrzeug Information<br/><br/><t color='#33CC33'><t align='left'><t size='1'>Halter: <t color='#ffffff'>%1<br/><t color='#33CC33'>naechster TUEV in: <t color='#ffffff'>%2 Monaten<br/>",_owners,_tuv];
