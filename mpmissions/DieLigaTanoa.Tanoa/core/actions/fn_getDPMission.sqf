...
if(str(_target) in LIGA_SETTINGS(getArray,"delivery_points")) then {
	private "_point";
	_point = LIGA_SETTINGS(getArray,"delivery_points");
	_point deleteAt (_point find (str(_target)));
	_dp = _point call BIS_fnc_selectRandom;
} else {
	_dp = LIGA_SETTINGS(getArray,"delivery_points") call BIS_fnc_selectRandom;
};
