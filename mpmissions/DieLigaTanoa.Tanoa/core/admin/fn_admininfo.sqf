...
_unit = _ret select 3;
ad_targ_uni = _unit;
ad_targ_id = _ret select 2;
if(__GETC__(life_adminlevel) < 3)exitWith{_text ctrlSetStructuredText parseText format["Name: %1<br/>UID: %2",_unit getVariable["realname",name _unit],_unit getVariable["steam64ID",name _unit]];};
