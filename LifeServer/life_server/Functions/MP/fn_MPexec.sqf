...
if (typename _target == typename []) then {
	...
	if (ismultiplayer && _mode isEqualTo 0) then {
		...
		if (_ownerID < 0) then {
			...
			if !(_ownerID isEqualTo _serverID) then {
				//--- Client
		...
		if (_ownerID isEqualTo -1 || _ownerID isEqualTo _serverID) then {
		...
		_canExecute = switch (typename _target) do {
			...
			case (typename sideUnknown): {(player call bis_fnc_objectside) isEqualTo _target};
			default {true};
		};
